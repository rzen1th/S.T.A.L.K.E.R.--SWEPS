...
function SWEP:PrimaryAttack()
    ...
    if self:Clip1() < self:GetProcessedValue("AmmoPerShot") then
        ...
        self:EmitSound(self:GetProcessedValue("SoundDryFire"), 75, 100, 1, CHAN_BODY)
        self:SetBurstCount(0)
        self:SetNeedTriggerPress(true)
        return
    end
    ...
    if self:RunHook("Hook_BlockFire") then return end
    ...
    self:GetOwner():DoAnimationEvent(self:GetProcessedValue("GestureShoot"))

    local pvar = self:GetProcessedValue("ShootPitchVariance")
    local pvrand = util.SharedRandom("STALKERPLUS_sshoot", -pvar, pvar)

    self:EmitSound(self:RandomChoice(self:GetProcessedValue("SoundShoot")) or "", self:GetProcessedValue("VolumeShoot"), self:GetProcessedValue("PitchShoot") + pvrand, 1, CHAN_WEAPON)

    self:EmitSound(self:RandomChoice(self:GetProcessedValue("DistantSoundShoot")) or "", 149, self:GetProcessedValue("PitchShoot") + pvrand, 1, CHAN_WEAPON)
    ...
    if self:GetProcessedValue("ShootEnt") then
        ...
        if IsFirstTimePredicted() then
            if GetConVar("stalkerplus_bullet_physics"):GetBool() then
                for i = 1, self:GetProcessedValue("Num") do
                    dir = self:GetOwner():EyeAngles() + (spread * AngleRand() / 3.6)
                    STALKERPLUS:ShootPhysBullet(self, self:GetOwner():GetShootPos(), dir:Forward() * self:GetProcessedValue("PhysMuzzleVelocity"))
                end
            else
...
function SWEP:AfterShotFunction(tr, dmg, range, penleft, alreadypenned)
    ...
    local bodydamage = self:GetProcessedValue("BodyDamageMultipliers")
    ...
    if GetConVar("STALKERPLUS_bodydamagecancel"):GetBool() and cancelmults[tr.HitGroup] then
    ...
    if self:GetOwner():IsNPC() and !GetConVar("STALKERPLUS_npc_equality"):GetBool() then
        dmgv = dmgv * 0.25
    end
