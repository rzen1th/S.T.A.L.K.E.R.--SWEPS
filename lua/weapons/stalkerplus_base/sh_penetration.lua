...
function SWEP:Penetrate(tr, range, penleft, alreadypenned)
    if !GetConVar("STALKERPLUS_penetration"):GetBool() then return end
    ...
    local penmult     = STALKERPLUS.PenTable[tr.MatType] or 1
    ...
    if penleft > 0 then
        ...
        if GetConVar("stalkerplus_bullet_physics"):GetBool() then
            STALKERPLUS:ShootPhysBullet(self, endpos, dir * self:GetProcessedValue("PhysMuzzleVelocity"), {
                Penleft = penleft,
                Travelled = range,
